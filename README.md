...
- **Doc windows say which document they hold.** Every gmist or Google Workspace window is titled with the file it was opened from, so the title bar, the taskbar hover and the window's own toolbar all name it. A new window steps down-right from the last one rather than opening on the same remembered frame, so two are never stacked exactly on top of each other.
