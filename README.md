...
**`.md` / `.txt`** (file rows): same editor + preview/autosave (UTF-8). **`.gdoc` / `.gsheet` / `.gslides`** (Google Drive for Desktop shortcuts): **Open** tries to load Docs/Sheets/Slides in a child window (persistent login partition). If the tiny JSON stub cannot be read (common under `.shortcut-targets-by-id` when Node sees `EISDIR`), TagFox on **Windows** reads the Drive virtual stream **`file.ext:user.drive.id`** to get the cloud file id and build the `docs.google.com` / `drive.google.com` URL. That id usually comes from the Drive FS driver, so it typically works for **streamed** (on-demand) files as well as **mirrored** ones—you do not need the full file hydrated on disk. If the stream is missing or the id is still a `local…` placeholder, use **Open** (OS handler), **Search Google Drive for filename…**, or open the file once in the browser so Drive registers it. **PDF** and other Office types where supported ([mammoth](https://github.com/mwilliamson/mammoth.js) / [SheetJS](https://sheetjs.com/) via CDN; legacy `.doc` stays "use Open"). Large files may not preview.
