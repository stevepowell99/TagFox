...
- **Open in gmist.** A markdown file (`.md`/`.qmd`) gets a row button that opens it in the [gmist](https://mist.broad-smoke-cc64.workers.dev) web editor. If a local gmist is running (`npm run dev:local`), TagFox opens the file straight from disk by its path, so any file on any drive works, in or out of Google Drive. Otherwise it opens the deployed editor using the file's Drive id (read from the local mirror), which needs the file to be in Google Drive. Auth is your signed-in gmist session.
