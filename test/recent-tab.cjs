// The global Ctrl+Shift+Space opens a tab on the last week's changes anywhere, reusing an existing
...
// guards what the tab is opened WITH and how often one is opened: empty query, no scope folder, no tag filters, 1 week recency,
...
async function main() {
  ...
  try {
    ...
    check('the reused tab is empty and on 1 week', reused.q === '' && reused.root === '' && reused.rec === '1w' && reused.cs === false, JSON.stringify(reused));
    ...
    check('pressing it in that tab clears the text and resets the rest', (await T('tabIds')).length === 2 && inPlace.q === '' && inPlace.rec === '1w' && inPlace.dia === false, JSON.stringify(inPlace));
